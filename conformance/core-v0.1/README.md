...
| §7 Integrity                 | All-revision content hashing in UTF-8 `id` then numeric `rev` order; duplicate revision rejection; metadata-only changes; unknown-key invariance; Unicode/body normalization; entry and clearance permutation invariance; path- and temporal-resolution-sensitive context IDs; bundle-failure state in context IDs; exact canonical effective-context bytes | TUF signing, `org.lock`, cryptographic delegation, whole-directory integrity, revocation, and freeze-horizon behavior are Extended and are not tested here |
